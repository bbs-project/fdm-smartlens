 ...
 * @param {Array[Number]} ratios boxes ratio [xRatio, yRatio], 경계 상자 비율 나타내는 배열
 */
export const renderYoloBoxes = async ( // 예측 경계 상자를 렌더링하는 역할
  ctx,
  threshold,
  boxes_data,
  scores_data,
  ...
  // console.log("[YOLO_0]=[", boxes_data, scores_data, classes_data, "]");
  // check if ctx is expo 2d context
  if (!ctx || !ctx.flush) {
    console.error("[renderYoloBoxes] ctx is not an Expo 2D context");
    return;
  }
  //ratios[1] = ratios[0]; // yRatio를 xRatio와 동일하게 설정
  let xratio = ratios[0];
  let yratio = ratios[1];
  ...
  for (let i = 0; i < scores_data.length; ++i) { // 실제로 예측 경계 상자 화면에 그리는 작업
    if (scores_data[i] > threshold) { // score가 threshold 보다 큰 경우에만 수행
      ...
      // Version 1
      // let [y1, x1, height, width] = boxes_data.slice(i * 4, (i + 1) * 4);
      // x1 = (x1 / xratio);
      // y1 = (y1 * xratio) - 100;
      // width *= yratio;      
      // height *= xratio; 
      ...
      // Version 2      
      // let [x1, y1, width, height] = boxes_data.slice(i * 4, (i + 1) * 4); // 현재 예측에 해당하는 경계 상자의 좌표를 추출      
      // // console.log("[renderYoloBoxes 1.5] x1, y1, x2, y2", x1, y1, x2, y2);
      // x1 *= ctx.width * ratios[0];
      // y1 *= ctx.height * ratios[1];
      // // x2 *= ctx.width * ratios[0];
      ...
      // flip horizontal
      // if (flipX) x1 = ctx.width - x1 - width; // flipX가 true 인 경우 경계 상자의 X 좌표를 화면 너비에서 좌표와 너비를 뺀 값으로 설정하여 수평으로 뒤집음
      // else x1 = x1;
      ...
      ctx.fillStyle = "#ffffff";
      ctx.fillStyle = "#000000";
      ctx.fillText(klass + " (" + score + "%)", x1 - 1, yText < 0 ? 0 : yText);// 레이블을 화면에 그림
    }
  }
